...
{
    public sealed class SubmitExchangeRateSymbolsConsumer : IConsumer<SubmitEcbExchangeRateSymbols>
    {
        ...
        public async Task Consume(ConsumeContext<SubmitEcbExchangeRateSymbols> context)
        {
            if (context.RequestId != null)
            {
                if (context.Message.Symbols?.Any() == true)
                {
                    ...
                    {
                        await context.RespondAsync<EcbExchangeRatesRejected>(new
                        {
                    ...
                    {
                        await context.RespondAsync<EcbExchangeRatesAccepted>(new
                        {
                            context.Message.EventId,
                            InVar.Timestamp,
                            context.Message.Symbols,
